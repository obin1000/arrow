...
#define DESERIALIZE(p)                                                                                                 \
   (((unsigned long long)(p)[0]) << 16) | (((unsigned long long)(p)[1]) << 8) | ((unsigned long long)(p)[2])
#define SERIALIZE(l, p)                                                                                                \
    ...
    */
   struct fsst_block {
      bool is_empty() const { return seq_num == 0; }
      static fsst_block get_empty() { return fsst_block{.seq_num = 0}; }
      size_t seq_num;
      size_t data_len;
   ...
   inline fsst_block decompress_block(fsst_block &block_data) {
      ...
      fsst_block decompressed{
            .seq_num = block_data.seq_num,
            .data_len = uncompressed_size_hdr,
            .data_stored = std::make_unique<unsigned char[]>(uncompressed_size_hdr),
      };


      fsst_decoder_t decoder;
      const size_t hdr = fsst_import(&decoder, block_data.data + FSST_UNCOMPRESSED_FIELD);
      fsst_decompress(&decoder, block_data.data_len - FSST_UNCOMPRESSED_FIELD - hdr,
                      block_data.data + FSST_UNCOMPRESSED_FIELD + hdr, uncompressed_size_hdr,
                      decompressed.data_stored.get());
      decompressed.data = decompressed.data_stored.get();
      return std::move(decompressed);
   }


   inline fsst_block compress_block(const fsst_block &data) {
      const size_t buffer_size = 4 * data.data_len;
      fsst_block block{};
      ...
      const auto num_compressed =
            fsst_compress(encoder, 1, &data.data_len, const_cast<const unsigned char **>(&data.data), buffer_size,
      ...
      const size_t total_size = compressedLen + hdr + FSST_BLOCKHEADER_SIZE;
      SERIALIZE(total_size, block.data_stored.get()); // block starts with size

      std::copy_n(reinterpret_cast<const unsigned char *>(&data.data_len), FSST_UNCOMPRESSED_FIELD,
                  block.data_stored.get() + FSST_BLOCKSIZE_FIELD); // followed by uncompressed size
   ...
   inline void read_blocks(std::ifstream &src, atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> *blocks_queue) {
      ...
      for (unsigned int block_number = 1; true; block_number++) {
         ...
         block.data = block.data_stored.get();
         block.data_len = block_size - FSST_BLOCKSIZE_FIELD;
         if (src.peek() != EOF) {
   ...
   inline size_t combine_results(atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> *write_queue,
   ...
   inline void combine_results(atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> *write_queue,
                               std::vector<unsigned char> *output_buf) {
      const auto size = combine_results(write_queue, output_buf->data());
   ...
   inline void read_data(std::ifstream &src, atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> *data_queue,
      ...
      for (unsigned int block_number = 1; true; block_number++) {
         ...
         if (num_read < block_size || src.peek() == EOF)
            break;
      }
   }

   inline void read_data(const unsigned char *src, const size_t src_size,
                         atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> *data_queue, const size_t block_size) {
      ...
      for (unsigned int block_number = 1; true; block_number++) {
         ...
         if (read_size < block_size || current_size >= src_size)
            break;
      }
   }

   inline void read_data(const std::vector<unsigned char> &src,
                         atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> *data_queue, const size_t block_size) {
   ...
   inline void compress_file(std::ifstream &src, std::ofstream &dst, size_t block_size) {
      ...
      readerThread.join();
      data_queue.push(fsst_block::get_empty());

      compressThread.join();
      blocks_queue.push(fsst_block::get_empty());

      writerThread.join();
   }


   inline size_t compress_buffer(const unsigned char *src, const size_t src_length, unsigned char *dst,
                                 const size_t block_size) {
      atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> data_queue{};
      atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> blocks_queue{};

      std::thread readerThread(
            [&src, &src_length, &data_queue, &block_size] { read_data(src, src_length, &data_queue, block_size); });
   ...
   inline void decompress_file(std::ifstream &src, std::ofstream &dst, const size_t number_of_threads) {
      ...
      write_queue.push(fsst_block::get_empty());
      writerThread.join();
   }
} // namespace fsstp
