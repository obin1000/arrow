...
namespace fsstp::no_queues {
   ...
   inline fsst_block_location decompress_block(fsst_block_location &block_data) {
      ...
      fsst_decompress(&decoder, block_data.data_len - hdr, block_data.data + hdr, block_data.uncompressed_size,
    ...
    * @param dst
    */
   inline size_t decompress_steps(const unsigned int thread_num, const unsigned int total_threads, const unsigned char *src,
                                  const size_t input_size, unsigned char *dst) {
      ...
      for (unsigned int block_num = 0; true; block_num++) {
         ...
         const auto size =
               fsst_decompress(&decoder, data_size, src + input_location, uncompressed_size, dst + output_location);
   ...
   inline size_t decompress_buffer(const unsigned char *src, const size_t input_size, unsigned char *dst,
      ...
      for (unsigned int thread_num = 0; thread_num < num_decomp_threads; thread_num++) {
         decomp_threads[thread_num] = std::async([thread_num, num_decomp_threads, &src, input_size, &dst]() -> size_t {
            return decompress_steps(thread_num, num_decomp_threads, src, input_size, dst);
         });
      }
