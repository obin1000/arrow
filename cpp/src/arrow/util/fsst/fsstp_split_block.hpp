...
namespace fsstp::split_block {
   ...
   inline void decompress_task_thread(atomic_queue::AtomicQueue2<fsst_decompression_task, QUEUE_SIZE> *input_queue,
      ...
      while (true) {
         ...
         fsst_block new_block{
            ...
            .data_len = 0,
            .data_stored = std::make_unique<unsigned char[]>(
                  4 * task.data_len) // TODO: Find better uncompressed length for split block
      };
         const auto decomp_size = fsst_decompress(task.decoder.get(), task.data_len, task.data, 4 * task.data_len,
                                                  new_block.data_stored.get());
    ...
    * @return
    */
   inline size_t decompress_buffer_tasks(const unsigned char *src, const size_t input_size, unsigned char *dst,
                                         const unsigned int number_of_threads, const unsigned int splits) {
      std::vector<std::thread> threads(number_of_threads);
      atomic_queue::AtomicQueue2<fsst_decompression_task, QUEUE_SIZE> tasks_queue;
      atomic_queue::AtomicQueue2<fsst_block, QUEUE_SIZE> write_queue;

      std::thread readerThread(
            [&src, &input_size, &tasks_queue, &splits] { read_blocks(src, input_size, &tasks_queue, splits); });

      for (unsigned int thread_num = 0; thread_num < number_of_threads; thread_num++) {
         threads[thread_num] =
               std::thread([&tasks_queue, &write_queue] { decompress_task_thread(&tasks_queue, &write_queue); });
      }
